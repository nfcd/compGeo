...
class Elastic_model:
    ...
    def plot(self):
        ...
        im0 = axs[0].imshow(self.vp, extent=[x0, xend, zend, z0], aspect=2, cmap='rainbow')
        axs[0].set_xlabel('X (m)')
        axs[0].set_ylabel('Z (m)')
        axs[0].set_title('P-wave velocity');
        cbar0=fig.colorbar(im0, ax=axs[0], orientation='horizontal')
        cbar0.ax.set_xlabel('Vp (m/s)')
        im1 = axs[1].imshow(self.vs, extent=[x0, xend, zend, z0], aspect=2, cmap='rainbow')
        axs[1].set_xlabel('X (m)')
        axs[1].set_title('S-wave velocity');
        cbar1=fig.colorbar(im1, ax=axs[1], orientation='horizontal')
        cbar1.ax.set_xlabel('Vs (m/s)')
        im2 = axs[2].imshow(self.rho, extent=[x0, xend, zend, z0], aspect=2, cmap='rainbow')
        axs[2].set_xlabel('X (m)')
        axs[2].set_title('Density');
        cbar2=fig.colorbar(im2, ax=axs[2], orientation='horizontal')
        cbar2.ax.set_xlabel('rho (kg/m3)')
        plt.show()
