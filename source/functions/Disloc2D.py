...
def Disloc2D(tip,base,slip,nu,obsx,obsy):
	...
	s1 = slip*cos(dip)
	s2 = slip*sin(dip)
	[ux_part1,uy_part1] = Displacement(tip[0],tip[1],s1,s2,nu,obsx,obsy)
	[ux_part2,uy_part2] = Displacement(base[0],base[1],-s1,-s2,nu,obsx,obsy)
...
def Displacement(xi1,xi2,s1,s2,nu,x1,x2):
	...
	if sign(s1)==sign(s2): 
		...
		s1 = -s1 
		s2 = -s2
	# These equations are written for xi1 = 0. If that's not the case, 
