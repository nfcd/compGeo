...
def GridFinStrain(pos,disp,frame,k,par,plotpar,plotst):
	'''
	GridFinStrain computes the finite strain of a group
	of points with displacements in x and y.
	Strain in z is assumed to be zero (plane strain)

	USE: cent,eps,pstrain,dilat,maxsh = GridFinStrain(pos,disp,frame,k,par,plotpar,plotst)
	...
	GridFinStrain uses functions lscov and FinStrain
	'''
	# If Delaunay
	if k == 0:
	...
	# Plot stations
	if plotst == 1:
		plt.plot(pos[:,0],pos[:,1],'k.',markersize=2) 
