...
def InfStrain(e):
	'''
	InfStrain computes infinitesimal strain from an input
	displacement gradient tensor

	USE: eps,ome,pstrain,rotc,rot = InfStrain(e)
	...
	pstrain[0,0] = D[2]
	pstrain[0,1],pstrain[0,2] = CartToSph(V[0,2],V[1,2],V[2,2])
	# Intermediate principal strain
	pstrain[1,0] = D[1] 
	pstrain[1,1],pstrain[1,2] = CartToSph(V[0,1],V[1,1],V[2,1])
	# Minimum principal strain
	pstrain[2,0] = D[0] 
	pstrain[2,1],pstrain[2,2] = CartToSph(V[0,0],V[1,0],V[2,0])

	# Calculate rotation components, Eq. 8.16
	rotc[0]=(ome[1,2]-ome[2,1])*-0.5
	rotc[1]=(-ome[0,2]+ome[2,0])*-0.5
	rotc[2]=(ome[0,1]-ome[1,0])*-0.5

	# Compute rotation magnitude, Eq. 8.17
	rot[0] = np.sqrt(rotc[0]**2+rotc[1]**2+rotc[2]**2)
	# Compute trend and plunge of rotation axis
	rot[1],rot[2] = CartToSph(rotc[0]/rot[0],rotc[1]/rot[0],rotc[2]/rot[0])
	# If plunge is negative
	if rot[2] < 0:
		rot[1] = ZeroTwoPi(rot[1]+np.pi)
		rot[2] = -rot[2]
		rot[0] = -rot[0]
