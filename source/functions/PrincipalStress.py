...
def PrincipalStress(stress,tX1,pX1,tX3):
    ...
    pX1 = plunge of X1
    tX3 = trend of X3
    pstress = 3 x 3 matrix containing the magnitude (column 1), 
              trend (column 2), and plunge (column 3) of the 
              maximum (row 1), intermediate (row 2), and 
              minimum (row 3) principal stresses
    dCp = 3 x 3 matrix with direction cosines of the principal 
          stress directions: Max. (row 1), Int. (row 2), 
          and Min. (row 3) with respect to North-East-Down
    ...
    V,D = np.linalg.eigh(stress)

    # Fill principal stress magnitudes
    pstress[0,0] = D[2,2] # Maximum principal stress
    pstress[1,0] = D[1,1] # Interm. principal stress
    pstress[2,0] = D[0,0] # Minimum principal stress


    # The direction cosines of the principal stresses are with
    # respect to the X1X2X3 stress coordinate system, so they 
    # need to be transformed to the NED coordinate system
    tV = np.zeros((3,3))
    for i in range(3):
        for j in range(3):
            for k in range(3):
